...
class Module(symbol: Symbol,
  ...
  private val dataArrs = dataShapes.map( x => x._1 -> argDict(x._1))
  private val labelArrs = labelShapes.map( x => x._1 -> argDict(x._1))
  ...
  argDict.foreach { case (name, ndArray) =>
    if (!dataLabelShape.contains(name)) {
      initializer(name, ndArray)
    }
  }
  ...
  def forward(datas: Map[String, NDArray], labels: Map[String, NDArray] = Map[String, NDArray]()): Unit = {
    datas.foreach { case (name, arr) => this.dataArrs(name).set(arr) }
    labels.foreach { case (name, arr) => this.labelArrs(name).set(arr) }
  ...
  def getParams(): (Map[String, NDArray], Map[String, NDArray]) = {
    (this.argDict.filter(x => !dataLabelShape.contains(x._1)), this.auxDict)
  }

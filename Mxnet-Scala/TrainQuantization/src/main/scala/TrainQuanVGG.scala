...
object TrainQuanVGG {
  ...
  private val logger = LoggerFactory.getLogger(classOf[TrainQuanVGG])
  
  def main(args: Array[String]): Unit = {
    val trgg = new TrainQuanVGG
    ...
    try {
      ...
      ResourceScope.using() {
        var sym = VggQuanSym.getQuanVgg16Symbol(numClasses = 10)
        ...
        module.initParams(initializer = new Xavier(rndType = "uniform", factorType = "avg", magnitude = 2.34f),
                          argParams = argParams, auxParams = auxParams, allowMissing = true)
        module.initOptimizer(optimizer = new Adam(learningRate = trgg.lr, wd = 0.0001f))

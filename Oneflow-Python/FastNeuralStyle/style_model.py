...
def conv2d_layer(
    ...
    output = flow.nn.conv2d(input, weight, strides, padding, data_format, dilation_rate, name=name)
    if use_bias:
...
def resBlock(input, channel, name_prefix, trainable = True):
    out = conv2d_layer(name_prefix + "_conv1", input, channel, kernel_size = 3, strides = 1, trainable = trainable)
    out = instance_norm(out, name_prefix + "_in1", trainable = trainable)
    out = flow.nn.relu(out)
    out = conv2d_layer(name_prefix + "_conv2", out, channel, kernel_size = 3, strides = 1, trainable = trainable)
    out = instance_norm(out, name_prefix + "_in2", trainable = trainable)
    return out + input

def deconv(input, out_channel, name_prefix, kernel_size = 4, strides = [2, 2], trainable = True):
    weight = flow.get_variable(
        name_prefix + "_weight",
...
def styleNet(input, trainable = True):
    with flow.scope.namespace("style_transfer"):
        ...
        conv1 = conv2d_layer("first_conv", input, 32, kernel_size = 9, strides = 1, trainable = trainable)
        in1 = instance_norm(conv1, "first_conv_in", trainable = trainable)
        in1 = flow.nn.relu(in1)
        conv2 = conv2d_layer("second_conv", in1, 64, kernel_size = 3, strides = 2, trainable = trainable)
        in2 = instance_norm(conv2, "second_conv_in", trainable = trainable)
        in2 = flow.nn.relu(in2)
        conv3 = conv2d_layer("third_conv", in2, 128, kernel_size = 3, strides = 2, trainable = trainable)
        ...
        conv1 = conv2d_layer("last_conv", in5, 3, kernel_size = 9, strides = 1, trainable = trainable)
